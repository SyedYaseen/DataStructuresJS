...
class Solution(object):
    ...
    def func(self, board):
        ...
        def elimination(board):
            emptyVal = {}
            square_values = [set() for i in range(9)]
            for row in range(9):
                row_values = set()
                col_values = set()
            ...
            while bool(startKeyList) and outerInitLen != outerLoopLen:
                ...
                for key in startKeyList:
                    ...
                    for i in range(9):
                        if board[i][col] != '.':
                            emptyVal[key].discard(board[i][col])

                        if board[row][i] != '.':
                            emptyVal[key].discard(board[row][i])
                ...
                for i in range(x, x + 3, 1):
                    for j in range(y, y + 3, 1):
                        if board[i][j] != '.':
                            emptyVal[key].discard(board[i][j])
        ...
        def pickValues(board):
            print("master", self.masterEmptyVal)
